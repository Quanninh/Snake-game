...
void end_game(int* cells, size_t width, size_t height, snake_t* snake_p) {
    // Game over!
// Free dynamic allocate memory for board cells
	if (cells)
	{
		free(cells);
		cells = NULL;
...
int main(int argc, char** argv) {
    ...
    if (status != INIT_SUCCESS) {
        switch (status) {
            ...
            case INIT_ERR_BAD_CHAR:
                printf("Error: Board contains invalid characters.\n");
                break;
            default:
                printf("Error: Unknown initialization error.\n");
                break;
        }
    free(cells); // Clean up allocated memory
    return EXIT_FAILURE;
    }
    ...
    // Wait before the next iteration (adjust speed with the value)
